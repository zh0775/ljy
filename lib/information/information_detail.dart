...
class InformationDetailBinding implements Bindings {
  @override
  void dependencies() {
    Get.put<InformationDetailController>(
        InformationDetailController(datas: Get.arguments));
  }
}

class InformationDetailController extends GetxController {
  final dynamic datas;
  InformationDetailController({this.datas});
  ...
  loadData({bool isLoad = false, int? loadIdx, String? searchText}) {
    ...
    Map<String, dynamic> params = {
      ...
      "countState": listStatus[myLoadIdx]["id"],
    };
    if (searchText != null && searchText.isNotEmpty) {
      params["devNo"] = searchText;
    }
    simpleRequest(
      url: Urls.userTradeDataList,
      params: params,
...
class InformationDetail extends GetView<InformationDetailController> {
  final int infoType;
  const InformationDetail({super.key, this.infoType = 0});
                ...
                child: Container(
                  ...
                  child: Stack(
                    children: [
                          ...
                          child: Row(
                            children: List.generate(
                              ...
                              return CustomButton(
                                ...
                                child: GetX<InformationDetailController>(
                                    builder: (_) {
                                  return SizedBox(
                                    width:
                      ...
                      GetX<InformationDetailController>(
                        builder: (_) {
  ...
  Widget list(int listIdx) {
    return GetBuilder<InformationDetailController>(
    ...
    return Align(
      child: Container(
        ...
        child: Column(
          children: [
            sbhRow([
              getSimpleText("交易额：￥${priceFormat(data["txnAmt"] ?? 0)}", 15,
                  AppColor.text2,
                  isBold: true),
            ...
            SizedBox(
              ...
              Column(
                ...
                children: [
                  ghb(6),
                  infoCell("订单编号", data["tradeNo"] ?? ""),
                  infoCell("设备编号", data["devNo"] ?? ""),
                  infoCell("型号", data["productName"] ?? ""),
                  infoCell("交易额", "${priceFormat(data["txnAmt"] ?? 0)}元"),
                  infoCell("手续费", "${priceFormat(data["txnFee"] ?? 0)}元"),
                  infoCell("商家费率", "${data["merName"] ?? "0"}%"),
                  infoCell("结算费率", "${data["termRate"] ?? "0"}%"),
                  infoCell("收益", "${priceFormat(data["bonusAmt"] ?? 0)}元"),
                  infoCell("交易时间", data["txnTime"] ?? ""),
